...
<!-- Generated by `claim-graphs skill --write` from docs/skill/SKILL.md and the declarations named in extract/claim_graphs/skill.py. Do not edit: edit the source or the declaration and regenerate. `make check` fails when this file is not what they generate. -->
...
| `role` | The work it does | Typical `claim-type` |
|:--|:--|:--|
| `hypothesis` | An answer the paper commits to, to a question it states: the proposition it bets on, phrased as a claim about the world rather than as the question | `hypothesis` |
| `prediction` | What should be observed if the hypothesis holds, under stated conditions | `prediction` |
| `empirical` | A measured or computed result, anchored to the panel that shows it, carrying the paper's own numbers and the paper's own epistemic verb | `empirical` |
| `control` | An empirical result whose work in the argument is to eliminate an alternative explanation or to show a manipulation did what it should | `empirical` |
| `scope` | A boundary condition on what the results can mean: the model class, the preparation, the population, the design | `assessment` |
| `methodological` | A capability or analytical commitment that a downstream result depends on for its interpretation: the sorting pipeline, the null distribution, the model fit that licenses a model-based analysis | `assessment` |
| `synthesis` | A higher-order proposition that integrates several of the paper's own results into one claim, staying inside the paper's evidence: the dissociation, the reconciliation, the summary that several panels jointly establish | `synthesis` |
| `interpretation` | A reading of the results through a theoretical lens from outside the paper's own evidence: a mapping onto a framework, a proposed mechanism, a functional meaning | `interpretive` |
| `literature-context` | A finding from cited prior work that the paper's argument inherits as a premise, recorded as a claim of its own so the inheritance is auditable | `interpretive` |
...
| `attributes` | someone else asserts it and this document reports that. Requires a `source` |
| `entertains` | raised as a candidate, not asserted. What a control's `rules-out` needs as its target |
...
`moderate`, `strong`, `tentative`, `weak` — the set `schema/to_claim_set.py` accepts when converting a claim file to the interchange form.

What `epistemic` *means* is an open question (`epistemic-vocab`, issue #20): on some claims it records how strongly the evidence supports the proposition, and on others it restates the role — a hypothesis marked `weak` because hypotheses are hypotheses, not because anything was measured. Record support strength, and do not read the corpus's existing values as consistent.
