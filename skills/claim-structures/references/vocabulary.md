...
<!-- Generated by `claim-graphs skill --write` from docs/skill/references/vocabulary.md and the declarations named in extract/claim_graphs/skill.py. Do not edit: edit the source or the declaration and regenerate. `make check` fails when this file is not what they generate. -->

This is the contract file the pipeline sends to every model call, reproduced here in full and
byte-identical. An agent working from this page and a model call made by `claim-graphs` are
given the same definitions and the same examples — that is the point of rendering it rather
than restating it.
