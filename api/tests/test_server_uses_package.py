...
def test_extract_endpoint_uses_package_run_agent():
    """The /extract endpoint calls claim_graphs.agents.run_agent, not a private copy."""
    _stub_heavy_imports()
    sys.modules.pop("server", None)
    sys.path.insert(0, str(API_DIR))
    try:
...
def test_extract_endpoint_uses_package_reconcile():
    """The /extract endpoint calls claim_graphs.reconcile.reconcile, not a private copy."""
    _stub_heavy_imports()
    sys.modules.pop("server", None)
    sys.path.insert(0, str(API_DIR))
    try:
...
def test_extract_endpoint_uses_package_external_review():
    """The /extract endpoint calls claim_graphs.external_review.external_review."""
    _stub_heavy_imports()
    sys.modules.pop("server", None)
    sys.path.insert(0, str(API_DIR))
    try:
