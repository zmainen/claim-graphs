...
# Add the extract package to the path — works installed or from a checkout
API_DIR = Path(__file__).resolve().parent
for candidate in [
    API_DIR.parent / "extract",                                         # published repo (extract/ at root)
    API_DIR / "claim_graphs",                                          # standalone deploy
]:
    if (candidate / "claim_graphs").is_dir() or candidate.name == "claim_graphs":
        sys.path.insert(0, str(candidate if candidate.name != "claim_graphs" else candidate.parent))
        break
else:
    # Last resort: assume claim_graphs is importable from current PYTHONPATH
    pass

# Prompts directory — configurable via env var


_prompts = os.environ.get("CLAIM_GRAPHS_PROMPTS_DIR")
PROMPTS_DIR = Path(_prompts).resolve() if _prompts else None
...
PROVIDER_MODELS = {
    ...
    # Hosted demo path (server-paid Vertex). Some regions pin opus at 4-6 —
...
def _ensure_pipeline():
    ...
    if not _pipeline_ready:
        ...
        from claim_graphs.prepare import prepare
        from claim_graphs.agents import run_agent, reset_client
        from claim_graphs.reconcile import reconcile as reconcile_step
        from claim_graphs.external_review import external_review
        from claim_graphs.edges import infer_edges, apply_oxa_edges
...
DEMO_TOKENS = set(
    t.strip() for t in (os.environ.get("CLAIM_GRAPHS_DEMO_TOKENS") or "").split(",")
    if t.strip()
)
# IPs that don't need a token (comma-separated in env var)
WHITELISTED_IPS = set(
    ip.strip() for ip in (os.environ.get("CLAIM_GRAPHS_WHITELIST_IPS") or "").split(",")
    if ip.strip()
)
...
app = FastAPI(
    title="Claim Graphs — Extraction API",
    description="Induce a typed claim graph from a scientific document",
    version="0.1.0",
)
    ...
    if PROMPTS_DIR and PROMPTS_DIR.is_dir():
        ...
        for p in [
            ...
            if p.is_dir():
                cfg.prompts_dir = p
                break
    cfg.output_dir = Path("/tmp/claim-graphs-api")
...
class _SSELogHandler(logging.Handler):
    ...
    `claim_graphs` logger.  Installing this handler on that logger during
...
def _run_with_sse(fn, q: "queue.Queue[str]", step: str):
    ...
    `claim_graphs` logger and forwarded to the SSE stream. The return value
    ...
    """
    pkg_logger = logging.getLogger("claim_graphs")
...
async def extract_file(
    ...
    # Save uploaded file
    upload_dir = Path("/tmp/claim-graphs-api/uploads")
    ...
    def generate():
        try:
            ...
            if suffix == ".pdf":
                from claim_graphs.prepare import extract_text, slice_sections, extract_figure_captions
                from claim_graphs.prepare import captions_text_block, guess_metadata, derive_slug
                from claim_graphs.prepare import PreparedPaper
                ...
                from claim_graphs.prepare import slice_sections, extract_figure_captions
                from claim_graphs.prepare import captions_text_block, guess_metadata, derive_slug
                from claim_graphs.prepare import PreparedPaper
