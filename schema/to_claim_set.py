...
SCHEMA = MACHINERY / "schema" / "claim-set-v0.schema.json"
...
def convert(doc: str) -> dict:
    index = frontmatter(CLAIMS / doc / "index.md") or {}
    ...
    for path in sorted((CLAIMS / doc).glob("*.md")):
...
def main() -> int:
    ...
    docs = [args.paper] if args.paper else sorted(
        Path(p).parent.name for p in glob.glob(str(CLAIMS / "*" / "index.md")))
    if args.out:
