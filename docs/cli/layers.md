...
`claim-graphs` is one subcommand per runnable layer. Each reads the paths its layer declares
and writes the path its layer declares, and does nothing else — no dependency resolution, no
ledger. [The runner](/claim-graphs/docs/runner/) does both.
...
|:-----------|:------|:---------|
| `prepare` | [`prepare`](/claim-graphs/pipeline/prepare/) | `runs/<paper>/prepared.json` |
| `results-reader` | [`results-reader`](/claim-graphs/pipeline/results-reader/) | `runs/<paper>/results-reader.output.json` |
| `caption-reader` | [`caption-reader`](/claim-graphs/pipeline/caption-reader/) | `runs/<paper>/caption-reader.output.json` |
| `structure-reader` | [`structure-reader`](/claim-graphs/pipeline/structure-reader/) | `runs/<paper>/structure-reader.output.json` |
| `reconcile` | [`reconcile`](/claim-graphs/pipeline/reconcile/) | `runs/<paper>/reconciler.output.json` |
| `external-review` | [`external-review`](/claim-graphs/pipeline/external-review/) | `runs/<paper>/external-review.output.json` |
| `edge-inference` | [`edge-inference`](/claim-graphs/pipeline/edge-inference/) | `runs/<paper>/edge-inference.output.json` |
| `write` | [`claim-tree`](/claim-graphs/pipeline/claim-tree/) | `claims/<paper>/*.md` |
| `verify-refs` | [`reference-check`](/claim-graphs/pipeline/reference-check/) | `runs/<paper>/reference-check.output.json` |
| `coverage` | [`coverage`](/claim-graphs/pipeline/coverage/) | `coverage/<paper>.json` |
| `mark` | [`marks`](/claim-graphs/pipeline/marks/) | `marked/<paper>.marked.md` |

`evaluate` is the twelfth subcommand and is not a layer; it has
[a page of its own](/claim-graphs/docs/evaluate/).
...
`~/.cache/claim-graphs/`, so re-running any layer on the same paper is free and offline.
...
```bash
claim-graphs results-reader   --paper <slug>
claim-graphs caption-reader   --paper <slug>
claim-graphs structure-reader --paper <slug>
```
...
```bash
claim-graphs reconcile --paper <slug>
claim-graphs reconcile --paper <slug> --reconcile-strategy union
```
...
```bash
claim-graphs external-review --paper <slug>
```
...
```bash
claim-graphs edge-inference --paper <slug>
...
```bash
claim-graphs reconcile --paper <slug> --dump-prompt /tmp/q.txt
# answer it anywhere — another model, another provider, a person
claim-graphs reconcile --paper <slug> --answer /tmp/a.json
```
...
```bash
claim-graphs write --paper <slug>
claim-graphs write --paper <slug> --format oxa    # one OXA JSON Document instead
```
...
claim-graphs verify-refs                            # sweep the corpus, write no report
```
...
claim-graphs coverage --paper <slug> --mapping ../mappings/<slug>.json
claim-graphs coverage --paper <slug> --fail-on-orphans
