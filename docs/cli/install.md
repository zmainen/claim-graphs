...
This installs the `claim-graphs` console script with its dependencies — `anthropic[vertex]`,
`httpx`, `pdfplumber`, `pyyaml`, `pydantic`.

```bash
claim-graphs --version
claim-graphs --help
```
...
export CLAIM_GRAPHS_BACKEND=anthropic
```
...
|:---------|:-----|:--------|
| `CLAIM_GRAPHS_ROOT` | `--root` | The corpus repository, which `pipeline/layers.yaml` resolves its paths against. Defaults to the directory the package ships in |
| `CLAIM_GRAPHS_CORPUS_DIR` | `--corpus-dir` | Where claim files are read and written (default: `<root>/claims`) |
| `VERTEX_PROJECT_ID` | `--vertex-project` | GCP project — **required**, there is no default |
...
| `CLAIM_GRAPHS_BACKEND` | `--backend` | Which provider to route to |
| `CLAIM_GRAPHS_MODEL_RESULTS` | `--model-results` | Override the Results-reader model |
| `CLAIM_GRAPHS_MODEL_CAPTION` | `--model-caption` | Override the Caption-reader model |
| `CLAIM_GRAPHS_MODEL_STRUCTURE` | `--model-structure` | Override the Structure-reader model |
| `CLAIM_GRAPHS_MODEL_RECONCILE` | `--model-reconcile` | Override the model for reconciliation, external review and edge inference |
...
```bash
cd extract
python3 -m claim_graphs.cli prepare --paper headley-2026-inhibitory-rhythms \
  --doi 10.7554/eLife.95562
python3 -m claim_graphs.cli results-reader --paper headley-2026-inhibitory-rhythms
```
...
| `root not found` | `--root` points somewhere that is not the repository | Pass `--root`, or set `CLAIM_GRAPHS_ROOT` |
...
- [One paper end to end](/claim-graphs/docs/first-paper/)
- [The runner](/claim-graphs/docs/runner/) — what `pipeline.py` does with all this
- [Configuration](/claim-graphs/docs/config/) — every flag in detail
