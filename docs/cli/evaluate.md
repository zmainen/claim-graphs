...
```bash
claim-graphs evaluate --reference-dir ../claims/ --work-dir /tmp/eval --all
claim-graphs evaluate --reference-dir ../claims/ --work-dir /tmp/eval --paper kammer-2026-foveal-feedback
claim-graphs evaluate --reference-dir ../claims/ --work-dir /tmp/eval --papers a,b,c
```
...
reviewer on the seven-bias prompt, against the ten papers of the reference corpus.
...
| Kind of paper | Role | What is hard about it |
|:--------------|-----:|:----------------------|
| a visual-neuroscience paper | 57% | Heavy `control` usage the reviewer prompt did not recognise |
| a methods paper | 65% | Prompts calibrated on biology findings fit a pipeline paper badly |
| a cognitive paper | 70% | `interpretation` versus `synthesis` is judgment-heavy here |

The papers are not named. Which paper scored what is a judgement about someone's work that
nothing here has adjudicated; what is worth carrying forward is the *kind* of paper and the
reason, because those are what a prompt revision has to answer to.
...
The worked example is the first row. Role agreement sat at 57% and the scorecard said why:
claims the reference marked `control` were coming back as `empirical`. One addition to the
external-reviewer prompt, teaching it to recognise implicit controls, moved that paper from
**57% to 71%** — and, re-run across the corpus, moved the aggregate role mean from 78.1% to
83.1% without costing anything on recovery.
...
that paper was kept as a known miss rather than iterated at until it passed.
...
That is a different question, and [`coverage`](/claim-graphs/docs/layers/) is what asks
