...
[`evaluate`](/claim-graphs/docs/evaluate/) scores a re-extraction against the committed
...
- **`prepare` is cached** under `~/.cache/claim-graphs/`, so re-running any layer on a paper
...
```bash
cd extract && python3 -m claim_graphs.cli prepare --paper <slug> --doi 10.7554/eLife.<id>
```
