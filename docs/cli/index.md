...
[the pipeline pages](/claim-graphs/pipeline/), where every figure is generated from the
...
**`claim-graphs`** is a set of layer runners — one subcommand per node in the graph that a
...
```
```
prepare          cd extract && python3 -m claim_graphs.cli prepare --paper gadeke-2026-guilt-insula …
results-reader   cd extract && python3 -m claim_graphs.cli results-reader --paper gadeke-2026-guilt-insula
caption-reader   …
structure-reader …
reconcile        …
external-review  …
...
- **[Install and configure](/claim-graphs/docs/install/)** — Python, credentials, the
  environment-variable contract.
- **[One paper end to end](/claim-graphs/docs/first-paper/)** — the walkthrough, from a
  DOI to a claim tree with a coverage report.
- **[The runner](/claim-graphs/docs/runner/)** — `pipeline.py`: `graph`, `backfill`,
  `state`, `run`, `approve`.
- **[The layer runners](/claim-graphs/docs/layers/)** — `claim-graphs`, one subcommand
  per layer.
- **[Batch operation](/claim-graphs/docs/batch/)** — running the corpus, and what it
  costs.
- **[Measuring a prompt change](/claim-graphs/docs/evaluate/)** — `evaluate`, the one
  subcommand that is not a layer.
- **[Configuration](/claim-graphs/docs/config/)** and
  **[the prompts](/claim-graphs/docs/prompts/)** — the reference tables.
- **[For contributors](/claim-graphs/docs/contributing/)** — code structure, adding a
...
| If you used | Use now |
|:------------|:--------|
| `claim-graphs extract --doi <doi>` | `pipeline.py run <paper> claim-tree`, or `prepare` then each reader |
| `claim-graphs run` | `pipeline.py run <paper> claim-tree` |
...
| | |
|:--|:--|
| [Papers](/claim-graphs/papers/) | the claim graphs themselves |
| [Pipeline](/claim-graphs/pipeline/) | the layer graph, and which papers have been through each layer |
| [Roles and relations](/claim-graphs/pipeline/vocabulary/) | what a claim can be, and what can hold between two |
| [Methodology](/claim-graphs/method/) | the procedure the corpus follows |
| [Design note](/claim-graphs/design/2026-09-11-layers-as-pipeline.html) | why the system is shaped this way |
