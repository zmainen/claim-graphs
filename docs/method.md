...
The reference corpus this method was developed against is **{{papers}} papers**, spanning one journal's neuroscience subfield mix. Two further papers — the two bioRxiv versions of one preprint — are carried as method examples rather than corpus members: they are the only pair here that shows how a claim tree changes between a preprint and its revision, and they are counted separately everywhere ({{method_example_claims}} claims across {{method_example_papers}} versions), never folded into a corpus total.
...
Per-paper counts are a fact about a corpus rather than about the method, so they live with the
corpus that has them; `scripts/corpus_facts.py` reports them for whichever corpus is present.
...
The fetch is cached under `~/.cache/claim-graphs/`, so every later layer on the same paper
