...
from claim_graphs import cli, contract, prompts, schema, vocabulary
from claim_graphs.config import Config
from claim_graphs.reconcile import _normalise_confidence

REPO = Path(__file__).resolve().parents[2]
PROMPTS = REPO / "extract" / "prompts"

# ── the corpus these tests need ───────────────────────────────────────────
#
# The prompt contract is not corpus-independent. It teaches each relation by rendering a real
# claim pair — twenty-odd claim files across five papers — so regenerating or verifying it reads
# claim files that live in the corpus repository, not here. The *rendered* contract under
# prompts/contract/ is committed and ships without a corpus; only checking it against its
# sources needs one.
#
# Point CLAIM_GRAPHS_CORPUS_DIR at a corpus checkout to run these. Whether the examples should
# instead be vendored here as reviewed prompt material is open — docs/design/2026-09-13-the-split.md § 4.
...
WHY_SKIPPED = (f"no corpus at {CORPUS}: the prompt contract's worked examples are real claim "
               f"files. Set CLAIM_GRAPHS_CORPUS_DIR to a corpus checkout.")
...
def test_committed_contract_matches_its_sources():
    ...
    assert not stale, f"regenerate with `claim-graphs contract --write`: {', '.join(stale)}"
...
def test_writer_numbers_a_shared_question_once():
    ...
    from claim_graphs import write
    from claim_graphs.schema import DraftClaimTable
