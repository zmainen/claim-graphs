...
# package beside it, so an `claim_graphs` installed from some other checkout wins. Test the
# tree you are standing in. pytest already does this via rootdir.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

import yaml

from claim_graphs import agents as agents_mod
from claim_graphs import cli, layers
from claim_graphs import reconcile as reconcile_mod
from claim_graphs.config import Config
from claim_graphs.prepare import PreparedPaper
from claim_graphs.schema import AgentExtraction, CandidateClaim, DraftClaimTable
...
def _read_paper() -> PreparedPaper:
    ...
    from claim_graphs.prepare import FigureCaption
    return PreparedPaper(
        doi=DOI, article_id="105391", paper_slug=SLUG, title="A paper", authors=["G"],
...
def test_every_declared_command_names_a_real_subcommand():
    ...
    for lid, layer in _declaration().items():
        ...
        m = re.search(r"claim_graphs\.cli\s+([a-z-]+)", cmd)
        if m:
...
def test_claim_tree_reads_edges_rather_than_re_inferring_them():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        from claim_graphs import write as write_mod
...
def test_the_dumped_prompt_is_the_one_the_layer_would_send():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        cfg = _cfg(Path(tmp))
        _seed_prepared(cfg.root, cfg)
        from claim_graphs.agents import build_reader_request, slice_for_agent, load_prompt
...
def test_evidence_found_normalises_curly_quotes_and_en_dash():
    ...
    from claim_graphs.agents import evidence_found
...
def test_reader_from_raw_sets_evidence_verified_when_paper_given():
    ...
    from claim_graphs.agents import reader_from_raw
...
def test_reconciled_claim_evidence_verified_has_one_entry_per_reader():
    ...
    from claim_graphs.reconcile import draft_from_raw
...
def test_spans_are_numbered_per_section_in_coverage_uid_format():
    ...
    from claim_graphs.prepare import _build_spans
    from claim_graphs.segment import segment
...
def test_results_reader_slice_carries_introduction_discussion_and_the_inventory():
    ...
    from claim_graphs.agents import slice_for_agent
...
def test_evidence_check_accepts_a_quote_against_the_cited_span():
    ...
    from claim_graphs.agents import _spans_by_uid, raw_slice_for_agent, verify_evidence
...
def test_stance_edge_validator_drops_a_rules_out_at_an_asserted_claim_and_a_bad_source():
    ...
    rival is kept."""
    from claim_graphs.edges import edges_from_raw
...
def _validate(parsed: list, claims: list | None = None) -> list[dict]:
    from claim_graphs.edges import _validate_edges
