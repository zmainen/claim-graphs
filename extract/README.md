...
writing per the claim schema (docs/claim-format.md § 4), CrossRef DOI verification, coverage measurement,
...
curated reference and 96% role agreement with `external-review` in the chain. The per-claim
scorecards behind those numbers are corpus evidence and live with the corpus, not here.

**Methodology authority:** `docs/method.md` § 3 (Claim induction — the ten-step process), § 4 (Schema). The CLI implements that methodology; when a behavior decision isn't anticipated by the doc, the CLI's behavior is the decision and the doc is updated in the same commit.
...
You also need Vertex AI credentials, and your own GCP project: there is no default project, so set `VERTEX_PROJECT_ID` (or `--vertex-project`) and `VERTEX_REGION`.
...
|:---------|:---------|:----------|:--------|
| `CLAIM_GRAPHS_ROOT` | `--root` | no | The corpus repository — what `pipeline/layers.yaml` resolves its paths against. Defaults to the directory this package ships in |
| `CLAIM_GRAPHS_CORPUS_DIR` | `--corpus-dir` | no | Where claim files are read/written (default: `<root>/claims`) |
| `VERTEX_PROJECT_ID` | `--vertex-project` | none | GCP project for Vertex AI Claude — required |
...
| `CLAIM_GRAPHS_MODEL_RESULTS` | `--model-results` | optional | Override Results-reader model |
| `CLAIM_GRAPHS_MODEL_CAPTION` | `--model-caption` | optional | Override Caption-reader model |
| `CLAIM_GRAPHS_MODEL_STRUCTURE` | `--model-structure` | optional | Override Structure-reader model |
| `CLAIM_GRAPHS_MODEL_RECONCILE` | `--model-reconcile` | optional | Override reconciliation + reviewer model |
| `CLAIM_GRAPHS_OUTPUT` | `--output-dir` | optional | Where draft tables and intermediates are written |
...
Fetches the paper from the eLife CDN (cached at `~/.cache/claim-graphs/`), slices it into
...
```bash
claim-graphs results-reader   --paper gadeke-2026-guilt-insula
claim-graphs caption-reader   --paper gadeke-2026-guilt-insula
claim-graphs structure-reader --paper gadeke-2026-guilt-insula
```
...
```bash
claim-graphs reconcile --paper gadeke-2026-guilt-insula
```
...
```bash
claim-graphs external-review --paper gadeke-2026-guilt-insula
```
...
```bash
claim-graphs edge-inference --paper gadeke-2026-guilt-insula
claim-graphs edge-inference --paper <slug> --dump-prompt /tmp/edges.txt
claim-graphs edge-inference --paper <slug> --edges-json /tmp/answer.json
```
...
```bash
claim-graphs write --paper gadeke-2026-guilt-insula
```
...
```bash
claim-graphs verify-refs --paper <slug>
claim-graphs verify-refs --paper <slug> --dry-run   # resolve, write nothing
```
...
```bash
claim-graphs coverage --paper <slug> --json ../coverage/<slug>.json
claim-graphs mark     --paper <slug> --mapping ../mappings/<slug>.json -o ../marked/<slug>.marked.md
```
...
```bash
claim-graphs evaluate \
  --reference-dir path/to/corpus/claims \
...
scorecard format as the round-trip harness that lives with the corpus. `--no-external-review`
...
defines it: `claim_graphs/vocabulary.py`, `scripts/relations.py`, `claim_graphs/schema.py`.
...
`hypothesis` roles — at a $2 / paper premium.
...
```
extract/
├── pyproject.toml               # package config, entry point: claim-graphs
├── README.md                    # this file
├── prompts/
...
│   └── contract/                # GENERATED — `claim-graphs contract --write`
...
└── tests/                      # the gated suite; per-paper scorecards live with the corpus
```
...
- [`../docs/method.md`](../docs/method.md) — the full methodology
- [`../docs/claim-format.md`](../docs/claim-format.md) — the claim format
- [`../docs/cli/`](../docs/cli/) — installing, configuring and running the CLI
