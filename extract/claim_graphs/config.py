...
# every generation. A Vertex project in europe-west1 typically has the 4.6
...
# ── Vertex AI defaults ───────────────────────────────────────────────────
#
# There is no default project, deliberately. It used to be one particular GCP project, so
# anyone who ran this without setting VERTEX_PROJECT_ID authenticated against somebody else's
# account — a silent operational hazard rather than a cosmetic leak. A missing project now
# fails with a message saying what to set.
DEFAULT_VERTEX_PROJECT = None
...
class Config:
    ...
    def from_args(cls, args) -> "Config":
        ...
        cfg.backend = (
            ...
            or os.environ.get("CLAIM_GRAPHS_BACKEND")
        ...
        cfg.model_results = _backend_model(cfg.backend,
            ...
            or os.environ.get("CLAIM_GRAPHS_MODEL_RESULTS")
        ...
        # Paths. The package ships inside the corpus repo as extract/claim_graphs/, so the
        # repository root is two directories up — the same guess prompts_dir already makes.
        root_arg = getattr(args, "root", None)
        root = (root_arg
                or os.environ.get("CLAIM_GRAPHS_ROOT")
        ...
        output = getattr(args, "output_dir", None) or os.environ.get("CLAIM_GRAPHS_OUTPUT")
    ...
    def validate(self) -> list[str]:
        ...
        if self.root is None or not self.root.is_dir():
            errors.append(
                f"root not found: {self.root}. Pass --root, or set CLAIM_GRAPHS_ROOT, "
