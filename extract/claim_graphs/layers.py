...
# ── what an answer cost, when a backend did not make the call ────────────

ANSWERED_TOKENS_ENV = "CLAIM_GRAPHS_ANSWERED_TOKENS"


def answered_usage() -> dict:
    """The usage block for a layer answered outside a backend call.

    An answer supplied through `--answer` costs the call path nothing, so `usage` was written
    empty and the ledger recorded how the answer arrived but not what it cost. `pipeline.py`
    has carried `--tokens` for exactly this since the answered path existed, and passed it in
    the environment because the declared command is fixed — but nothing here read it, so the
    flag was dead on the one path it was built for. Found by the first subagent run of the
    chain, which reported 122,680 tokens and recorded none.

    Not `input_tokens`: a session total covers reading the prompt, the contract and the corpus,
    then fixing what the validator refused. Summing it into the API's four fields would make
    two different things look like one measurement.
    """
    raw = (os.environ.get(ANSWERED_TOKENS_ENV) or "").strip()
    return {"answered_tokens": int(raw)} if raw.isdigit() and int(raw) else {}
...
def parts_layer(paper: str, cfg: Config, *,
    ...
    if answer is not None:
        p, model = answer_file(answer, cfg)
        raw = p.read_text(encoding="utf-8")
        parts_usage: dict = answered_usage()
    else:
        model = cfg.model_reconcile
...
def stance_layer(paper: str, cfg: Config, *,
    ...
    if answer is not None:
        p, model = answer_file(answer, cfg)
        raw = p.read_text(encoding="utf-8")
        stance_usage: dict = answered_usage()
    else:
        model = cfg.model_reconcile
...
def warrant_layer(paper: str, cfg: Config, *,
    ...
    if answer is not None:
        p, model = answer_file(answer, cfg)
        raw = p.read_text(encoding="utf-8")
        warrant_usage: dict = answered_usage()
    else:
        model = cfg.model_reconcile
...
# `--dump-prompt`/`--answer` seam: the rule (extract/claim_graphs/verification_check.py) is
...
def abstract_map_layer(paper: str, cfg: Config, *,
    ...
    if answer is not None:
        p, model = answer_file(answer, cfg)
        raw = p.read_text(encoding="utf-8")
        amap_usage: dict = answered_usage()
    else:
        model = cfg.model_reconcile
