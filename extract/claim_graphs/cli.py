...
def cmd_contract(args: argparse.Namespace) -> int:
    ...
    if stale:
        print(f"error: extract/prompts/{CONTRACT_DIR}/ is not what its sources generate: "
              f"{', '.join(stale)}\n  run: cd extract && python3 -m claim_graphs.cli contract --write",
              file=sys.stderr)
        return 1
...
def cmd_skill(args: argparse.Namespace) -> int:
    """Render the agent-facing skill from its sources, or check the committed one is current.

    Not a layer: it produces what an agent is told, from the same declarations the layers send
    to a model. Drift here is not cosmetic — a skill that names a relation the vocabulary
    retired teaches an agent to write a graph the gate rejects.
    """
    from .skill import SKILL_DIR, check, write

    cfg = _cfg(args)
    if args.write:
        for p in write(cfg.root):
            print(f"  wrote {p.relative_to(cfg.root)}")
    stale = check(cfg.root)
    if stale:
        print(f"error: {SKILL_DIR}/ is not what its sources generate: {', '.join(stale)}\n"
              f"  run: cd extract && python3 -m claim_graphs.cli skill --write",
              file=sys.stderr)
        return 1
    print(f"  {SKILL_DIR}/ is current")
    return 0
...
def _add_backend_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help=("Model backend: vertex (default), anthropic, openrouter, openai, "
              ...
              "is routed via litellm. Or set CLAIM_GRAPHS_BACKEND."),
    )
    parser.add_argument(
...
def _add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help="The corpus repository — what pipeline/layers.yaml resolves its paths against. "
             "Defaults to the directory this package ships in, or CLAIM_GRAPHS_ROOT.",
    )
    parser.add_argument(
...
def _add_model_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--vertex-project", default=None,
                        help="Vertex AI project ID (or set VERTEX_PROJECT_ID; there is no default).")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"claim-graphs {__version__}")
    ...
    p_skill = sub.add_parser(
        "skill", help="Render the agent-facing skill, or check that the committed one is current.",
        description=(
            "The skill an agent loads, rendered from docs/skill/ with every declared "
            "enumeration substituted from its declaration: relations and their directions from "
            "scripts/relations.py, the enforced rules from scripts/check_relations.py, roles "
            "and claim types from vocabulary.py, the sequence from pipeline/layers.yaml, and "
            "the vocabulary the model is sent from the prompt contract. --write regenerates "
            "skills/claim-structures/; without it the command only checks, and exits non-zero "
            "when the committed files are stale. Needs a corpus, like the contract does: the "
            "examples are real claims."
        ),
    )
    p_skill.add_argument("--write", action="store_true", help="Regenerate the skill files.")
    _add_common_args(p_skill)
