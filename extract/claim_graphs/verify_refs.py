...
# CrossRef asks callers to identify themselves, and gives a faster pool to those who do. Set
# CLAIM_GRAPHS_CROSSREF_MAILTO to your own address to join it. There is no default: a personal
# address hardcoded in a public repository is an address that gets scraped, and it would identify
# whoever published the code as the caller for every user of it.
CROSSREF_MAILTO = os.environ.get("CLAIM_GRAPHS_CROSSREF_MAILTO")
CROSSREF_USER_AGENT = (
    f"claim-graphs/0.1.0 (mailto:{CROSSREF_MAILTO})" if CROSSREF_MAILTO
    else "claim-graphs/0.1.0"
)
