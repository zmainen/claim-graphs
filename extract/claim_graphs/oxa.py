...
See: the OXA adoption spec (not public)
See: the OXA adoption spec (not public)
"""
...
class Claim(BaseModel):
    ...
    OXA RFC: the OXA adoption spec (not public)
    """
    model_config = ConfigDict(strict=True)
...
#
# Spec:        the OXA adoption spec (not public)  (hetu)
# Consumed by: the OXA adoption spec (not public)   (mimamsa)
...
class Reviewer(BaseModel):
    """The authoring party. Persona-grounded where the deployment has personas; an Agent (ORCID/DID) in Mira."""
    model_config = ConfigDict(strict=True)
...
# Relation -> CiTO IRI. The table is standards.yaml; `dissociates-with` is absent from it
# deliberately (#125) — CiTO has no neutral predicate for a contrast, and
# `cito:disagreesWith` misstates one as disagreement. The formats report names the omission.
EDGE_MAP: dict[str, str] = _standards.cito()
...
def claim_from_reconciled(
    ...
    Maps the claim-graphs vocabulary to OXA types.
    """
    panels = None
    if panel:
