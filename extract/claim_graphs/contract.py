...
the same for every role and is rendered here from the code that already defines it:
    ...
    claim_graphs/schema.py          the pydantic models the output is parsed into
...
def claims_dir(root: Path) -> Path:
    """Where the claim files are, which after the split need not be under `root`.

    The contract renders a real claim pair per relation, so building or checking it reads the
    corpus. This package no longer contains one: CLAIM_GRAPHS_CORPUS_DIR points at a checkout,
    and `root/claims` remains the default for a repository that does hold both.
    """
    explicit = os.environ.get("CLAIM_GRAPHS_CORPUS_DIR")
    return Path(explicit).expanduser().resolve() if explicit else Path(root) / "claims"


def _claim(root: Path, paper: str, slug: str) -> dict:
    p = claims_dir(root) / paper / f"{slug}.md"
    if not p.is_file():
...
def render_vocabulary(root: Path | None = None) -> str:
    ...
    w("")
    w("Generated from `extract/claim_graphs/vocabulary.py` and `scripts/relations.py` by "
      "`claim-graphs contract --write`. Do not edit; edit the source and regenerate. Every "
...
def render_schema_candidate() -> str:
    out = ["# What a reader returns", "",
           "Generated from `extract/claim_graphs/schema.py` by `claim-graphs contract --write`. "
           "Do not edit.", "",
