#!/usr/bin/env python3
"""Ingest OXA claim documents into a claims table.

Usage:
  python3 ingest_oxa_claims.py <oxa-json-file> [--db <sqlite-file>] [--dry-run]
...
# No default: the previous one pointed at a private database on one machine, so running this
# script unset either failed confusingly or touched somebody else's data.
DEFAULT_DB = None
...
def main():
    parser = argparse.ArgumentParser(description="Ingest OXA claims into a sqlite claims table")
    parser.add_argument("oxa_file", type=Path, help="OXA Document JSON file")
    parser.add_argument("--db", type=Path, default=DEFAULT_DB, help="sqlite database path (required)")
