...
CLAIMREL_NS = "https://w3id.org/claim-relations#"
...
# our own namespace, matching `cg:stance` in the MIRA export.
EXT_NS = "https://w3id.org/claim-graphs/schema#"
...
# CiTO IRI -> one of DG's four predicates, composed from standards.yaml. Keyed on CiTO because
# this reads OXA documents and OXA stores the CiTO IRI in `relationType`. Composing it rather
# than keeping a third hand-written map is what drops a dead entry automatically:
# `cito:disagreesWith` sat in the old map long after the #125 ruling stopped any relation
# mapping to it.
#
# A relation absent from here is one DG cannot express; it is carried as an annotation.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from claim_graphs import standards as _standards  # noqa: E402

RELATION_TO_DG = _standards.discourse_graphs_by_cito()
...
def oxa_to_jsonld(oxa_path: Path) -> dict:
    ...
    # Documents are identified relative to the graph that holds them, not to a domain this project
    # does not own. The old value was `elife-claim-trees.org`, which does not resolve and which
    # anyone could register and point anywhere — while being stamped into every export.
    base_uri = f"urn:claim-graphs:{paper_id}/"
    ...
    for claim in iter_claims(doc):
        ...
        if claim.get("stance"):
            node[f"{EXT_NS}stance"] = claim["stance"]
        if claim.get("stanceSource"):
            node[f"{EXT_NS}stanceSource"] = claim["stanceSource"]
