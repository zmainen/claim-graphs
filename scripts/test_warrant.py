...
# The dossier tests below read a real tree. `load_paper` exits the process when the paper is
# absent rather than raising, so without this guard a corpus-free run does not fail — it
# truncates, and the synthetic cases after it never report. Point CLAIM_GRAPHS_CORPUS_DIR at a
# corpus checkout to run them.
HAVE_CORPUS = os.path.isdir(os.path.join(warrant.CLAIMS_DIR, PAPER))
WHY_SKIPPED = (f"no corpus at {warrant.CLAIMS_DIR}: set CLAIM_GRAPHS_CORPUS_DIR to a checkout")
