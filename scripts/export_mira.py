...
the article platform this targets ingests. This writes two files:

  <slug>.mira.jsonld           strict MIRA core — safe to hand to any MIRA reader
  <slug>.mira-extended.jsonld  the same graph plus a `cg:` namespace carrying
...
# The corpus need not live under ROOT any more: this repository holds the machinery and the
# corpus is a separate checkout. Every script here reaches the claim files through this name, so
# pointing CLAIM_GRAPHS_CORPUS_DIR at a corpus is enough to make all of them work against it.
CLAIMS_DIR = (os.environ.get("CLAIM_GRAPHS_CORPUS_DIR")
              or os.path.join(ROOT, "claims"))
...
EXT = "https://w3id.org/claim-graphs/schema#"
...
# The package lives beside the runners, in the machinery checkout — not under ROOT, which
# after the split names the graph.
sys.path.insert(0, os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "extract"))
from claim_graphs import claimfile as _claimfile  # noqa: E402
from claim_graphs import standards as _standards  # noqa: E402

# Relation -> its MIRA property, domain, range and the note saying why, from standards.yaml.
RELATION_DEFS = _standards.mira_defs()
...
def load_frontmatter(path):
    """Parse a claim file's YAML frontmatter. See claim_graphs.claimfile for the one parser."""
    return _claimfile.load(path)
...
def build_node(claim, by_slug, extended):
    ...
    node["cgx:epistemicStatus"] = epistemic_status(role, claim.get("epistemic"))

    # ── cg: extension — everything MIRA cannot express ──────────────────
    ext = {"cg:role": role}
    ...
    if claim.get("epistemic"):
        ext["cg:epistemicStrength"] = claim["epistemic"]
    ...
    if typed:
        ...
        ext["cg:relations"] = {k: typed[k] for k in sorted(typed)}

    reps = []
    for r in (claim.get("reproductions") or []):
        rec = {
            "@type": "cg:VerificationRecord",
            "cg:agent": r.get("agent"),
            "schema:date": str(r.get("date")) if r.get("date") else None,
            "cg:status": r.get("status"),
            "cg:executed": r.get("script_execution") == "executed",
        }
        for src, dst in (("script", "cg:script"),
                         ("function", "cg:function"),
                         ("data_source", "cg:dataSource"),
                         ("data_commit", "cg:dataCommit"),
                         ("data_file", "cg:dataFile"),
                         ("paper_value", "cg:publishedValue"),
                         ("reproduced_value", "cg:reproducedValue")):
            if r.get(src):
                rec[dst] = r[src]
        reps.append({k: v for k, v in rec.items() if v is not None})
    if reps:
        ext["cg:verification"] = reps

    if a.get("panel"):
        ext["cg:panel"] = a["panel"]
    if a.get("analysis"):
        ext["cg:analysis"] = a["analysis"]
...
def synthesize_questions(claims, by_slug, extended, paper_slug=None, stated=None):
    ...
    and the link is the one the paper drew, carried without the `cg:autoDerived` flag. Only a
    ...
    if stated:
        ...
        for c in claims:
            ref = c.get("addresses")
            if ref:
                links[c["slug"]] = f"cg:question/{paper_slug}/{ref}"
                addressed.add(ref)
        for q in stated:
            questions.append({
                "@id": f"cg:question/{paper_slug}/{q['id']}",
                "@type": "mira:Question",
...
def _derived_question(c, extended):
    ...
    # `cg:` prefix is undeclared, so these keys would be undefined terms.
    # The gap report lists every derived question for review either way.
    if derived and extended:
        q["cg:autoDerived"] = True
        q["cg:reviewNote"] = ("Derived mechanically from the hypothesis text; "
...
def _rel_ids(key):
    ...
    safe = key.replace("-", "_")
    return f"cg:reldef/{safe}", f"cg:reldef/{safe}/def"
...
NODE_TYPES = {"mira:Claim": "cg:type/Claim",
              "mira:Evidence": "cg:type/Evidence",
              "mira:Protocol": "cg:type/Protocol"}
...
def export(paper_slug, extended):
    ...
    author_id = "urn:claim-graphs:pipeline"
        ...
        for key, target in relations(c):
            ...
            if t:
                ...
                if target not in minted:
                    minted[target] = f"cg:claim/{paper_slug}/alternative/{len(minted) + 1}"
                dest, dest_label = minted[target], target
            used.add(key)
            n += 1
            abstract, _ = _rel_ids(key)
            edges.append({
                "@id": f"cg:edge/{paper_slug}/{n}",
                "@type": [abstract],
                "source": node_id(c),
                "destination": dest,
    ...
    for text, mid in minted.items():
        ...
        if extended:
            alt["cg:materialisedFrom"] = "relation-target"
            alt["cg:reviewNote"] = (
    ...
    nodes.insert(0, {"@id": author_id, "@type": "UserAccount",
                     "accountName": "claim-graphs pipeline"})
    ...
    if extended:
        ctx["@context"].append({"cgx": "https://w3id.org/claim-graphs/schema-extended#"})
...
def gap_report(paper_slug, claims, dropped, questions, minted=(), wildcards=()):
    ...
    L.append(f"**{reps} verification records across {verified} claims are absent from the "
             ...
             f"They are carried in the extended file as `cg:VerificationRecord`.\n")
    ...
    if nonassert:
        ...
        L.append("The strict export still carries the `rules-out` edge that eliminated each "
                 ...
                 "The stance travels in the extended file as `cg:stance`.\n")
        for c in nonassert:
    ...
    if minted:
        L.append(f"**{len(minted)} relation targets name something no claim file asserts.** "
                 ...
                 f"`cg:materialisedFrom: relation-target` in the extended file. **None of "
